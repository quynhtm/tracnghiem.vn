        ...
        <div class="row">
                    ...
                    <div class="panel line marginTop5" id="element">
                        ...
                        <table class="table table-bordered bg-head-table">
                            ...
                            <tr>
                                ...
                                <th width="20%">{{viewLanguage('Thông tin Ngân hàng')}}</th>
                                <th width="10%" class="text-center">{{viewLanguage('BankId')}}</th>
                                <th width="30%">{{viewLanguage('Mô tả')}}</th>
                                <th width="10%" class="text-center">{{viewLanguage('24/7')}}</th>
                            ...
                            @if(isset($data) && sizeof($data))
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">@if($arrTypeItem[$item->type_item]){{$arrTypeItem[$item->type_item]}}@endif</td>
                                    ...
                                    <td class="text-center middle" align="center">
                                        @if($is_root || $permission_full || $permission_create)
                                            <a class="editItem" onclick="VM.editItem('{{$item->id}}', WEB_ROOT + '/manager/defineBank/ajaxLoad')" title="{{viewLanguage('Sửa')}}">
                                                <i class="fa fa-edit fa-2x"></i>
                                            </a>
                                        @endif
                                        @if($is_boss)
                                            <a href="javascript:void(0);" onclick="VM.deleteItem('{{$item->id}}', WEB_ROOT + '/manager/defineBank/delete')" title="{{viewLanguage('Xóa')}}">
            ...
            <div class="col-md-3 panel-content loadForm">
                <div class="panel panel-primary">
                    <div class="panel-heading paddingTop1 paddingBottom1">
                        <h4>
                            <i class="fa fa-edit icChage" aria-hidden="true"></i> <span class="frmHead">{{viewLanguage('Thêm mới ngân hàng')}}</span>
                        </h4>
                    </div>
                    <div class="panel-body">
                        <form id="formAdd" method="post">
                            ...
                            <div class="form-group col-lg-12">
                                <label for="define_name">{{viewLanguage('Tên ngân hàng')}} <span class="red"> (*) </span></label>
                                <input name="define_name"  id="define_name" title="{{viewLanguage('Tên ngân hàng')}}" class="form-control input-required"type="text">
                            </div>
                            <div class="form-group col-lg-12">
                            ...
                            <div class="form-group col-lg-6">
                                <label for="define_status">{{viewLanguage('Bank Id')}} <span class="red"> (*) </span></label>
                                <input name="object_id"  id="object_id" title="{{viewLanguage('Bank Id')}}" class="form-control input-required" type="text" value="">
                            </div>
                            <div class="form-group col-lg-6">
                                <label for="define_status">{{viewLanguage('Ngân hàng 24/7')}}</label>
                                <select class="form-control input-sm" name="type_item" id="type_item">
                                    {!! $optionTypeItem !!}
                                </select>
                            </div>
                            ...
                            <div class="form-group col-lg-12">
                                @if($is_root || $permission_full || $permission_create)
                                <a class="btn btn-success" id="submit" onclick="VM.addItem('form#formAdd', 'form#formAdd :input', '#submit', WEB_ROOT + '/manager/defineBank/post/0')">
