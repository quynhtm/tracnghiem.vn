...
use App\Http\Models\Admin\Careers;
...
class AdminCareerController extends BaseAdminController
{
    private $error = array();
    ...
    {
        parent::__construct();
        CGlobal::$pageAdminTitle = 'Quản lý Nghề nghiệp';
    }
    ...
    public function _getDataDefault()
    {
        ...
        $this->viewPermission = [
            ...
            'permission_full' => $this->checkPermiss(PERMISS_CAREER_FULL),
            'permission_create' => $this->checkPermiss(PERMISS_CAREER_CREATE),
            'permission_delete' => $this->checkPermiss(PERMISS_CAREER_DELETE),
        ];
    }

    public function view()
    {
        if (!$this->checkMultiPermiss([PERMISS_CAREER_FULL, PERMISS_CAREER_VIEW])) {
        ...
        //$search['field_get'] = '';
        $data = app(Careers::class)->searchByCondition($search, $limit, $offset, $total);
        ...
        return view('admin.AdminCareer.view', array_merge([
            'data' => $data['data'],
            'search' => $search,
            'total' => $total,
            'stt' => ($pageNo - 1) * $limit,
    ...
    public function postItem($id)
    {
        ...
        if ($this->_validData($data) && empty($this->error)) {
            ...
            if ($id > 0) {
                app(Careers::class)->updateItem($id, $data);
            } else {
                app(Careers::class)->createItem($data);
            }
        }
        $_data['url'] = URL::route('admin.careerView');
    ...
    public function deleteItem()
    {
        ...
        if (!$this->checkMultiPermiss([PERMISS_CAREER_FULL, PERMISS_CAREER_DELETE])) {
            return Response::json($data['msg'] = 'Bạn không có quyền thao tác.');
        }
        $id = (int)Request::get('id', 0);
        if ($id > 0 && app(Careers::class)->deleteItem($id)) {
            $data['isIntOk'] = 1;
        }
    ...
    public function ajaxLoadForm()
    {
        ...
        $id = $_POST['id'];
        $data = (($id > 0)) ? app(Careers::class)->getItemById($id) : [];

        $this->_getDataDefault();
        $optionStatus = getOption($this->arrStatus, isset($data['status']) ? $data['status'] : STATUS_SHOW);
        return view('admin.AdminCareer.component.ajax_load_item',
            array_merge([
                'data' => $data,
