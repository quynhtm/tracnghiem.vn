...
class AdminRoleController extends BaseAdminController
{
    ...
    public function view()
    {
        ...
        $data = app(Role::class)->searchByCondition($search, $limit, $offset);
        $paging = '';
    ...
    public function addRole($ids)
    {
        ...
        if ($this->valid($data) && empty($this->error)) {
            $id = ($id == 0) ? $id_hiden : $id;
            if ($id > 0) {
                app(Role::class)->updateItem($id, $data);
            } else {
                app(Role::class)->createItem($data);
            }
            $arrSucces['isOk'] = 1;
    ...
    public function ajaxLoadForm()
    {
        ...
        $data['role_id'] = 0;
        if ($id > 0) {
            $data = app(Role::class)->getItemById($id);
        }
        $this->getDataDefault();
